...
export function SteeringWheel({ size = 150, maxRotation = 540, onChange }: Props) {
  ...
  return (
    <View style={[styles.container, { width: size, height: size }]} {...panResponder.panHandlers}>
      ...
      >
        <View style={{ position: 'absolute', left: half - hubSize / 2, top: half - hubSize / 2, width: hubSize, height: hubSize, borderRadius: hubSize / 2, backgroundColor: '#2a2a2a', borderWidth: 2, borderColor: '#000' }} />
        {[0, 120, 240].map((deg) => (
          <View key={deg} pointerEvents="none" style={{ position: 'absolute', left: half - spokeW / 2, top: ringW + 4, width: spokeW, height: half - ringW - 4, backgroundColor: '#1a1a1a', transform: [{ translateY: half - ringW - 4 }, { rotate: `${deg}deg` }, { translateY: -(half - ringW - 4) / 2 }] }} />
        ))}
        <View pointerEvents="none" style={{ position: 'absolute', left: half - 6, top: 2, width: 12, height: ringW + 4, backgroundColor: '#ffd24a', borderRadius: 3 }} />
      </Animated.View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  wheel: { backgroundColor: 'rgba(40,40,40,0.9)', borderColor: '#1a1a1a' },
});
